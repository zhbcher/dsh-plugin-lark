...
function bindApp(appId, appSecret) {
  return new Promise((resolve) => {
    const child = spawn(LARK_CLI, ["config", "init", "--app-id", appId, "--brand", "feishu"], {
...
function startBotCreate(ctx) {
  ...
  try { mkdirSync(BOOTSTRAP_DIR, { recursive: true }); } catch { /* ignore */ }
  const eventsPath = join(BOOTSTRAP_DIR, "events.jsonl");
  try { rmSync(eventsPath, { force: true }); } catch { /* ignore */ }
  try { rmSync(join(BOOTSTRAP_DIR, "result.json"), { force: true }); } catch { /* ignore */ }
  ...
  // 默认跳过自动发布：工具的"发布"步骤在现行控制台存在定位 bug（"更新说明"输入框
  // 被误定位为禁用复选框导致点击超时、全流程失败）。跳过发布后流程可稳定跑完并提取凭证，
  // 发布上线改为在控制台手动完成（更可靠）。设 LARK_BOT_PUBLISH=true 可恢复自动发布。
  if (process.env.LARK_BOT_PUBLISH !== "true") args.push("--no-publish");
