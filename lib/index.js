...
// 通用默认：包内自带网关（bridge/）；本机可用 LARK_BRIDGE_DIR 指向已有网关目录
const BRIDGE_DIR = process.env.LARK_BRIDGE_DIR ?? join(dirname(dirname(fileURLToPath(import.meta.url))), "bridge"); // 包根目录下的 bridge/
...
/** 启停 lark-bridge 网关（转发给 manage_lark_bridge.sh）。action: "start" | "stop" */
async function listenerAction(ctx, action) {
  const script = join(BRIDGE_DIR, "manage_lark_bridge.sh");
  if (!existsSync(script)) return { ok: false, error: `找不到 ${script}` };
  ...
  return { ok: true, action, output: (stdout + stderr).trim() };
}
  ...
  return {
    ...
    start(deviceCode) {
      ...
      child.on("close", (code) => {
        ...
        // 授权成功后自动启动监听器，实现「扫码即连接」的完整体验
        if (code === 0) {
          listenerAction(ctx, "start").then((r) => {
            ctx.logger.info(`lark-connector: 授权成功，自动启动监听器 → ${r.output || r.error || "ok"}`);
          }).catch((e) => {
            ctx.logger.warn(`lark-connector: 自动启动监听器失败: ${e.message ?? String(e)}`);
          });
        }
      });
...
function startBotCreate(ctx) {
  ...
  botCreate = {
    ...
    result: null, error: null, bound: null, connected: false,
    ...
    try {
      ...
      if (code === 0 && resultPath && existsSync(resultPath)) {
        ...
        if (rj.status === "succeeded" && rj.credentials?.appId && rj.credentials?.appSecret) {
          ...
          // 绑定成功后自动启动监听器 → 「扫一次码即连接」（与 Hermes/OpenClaw 体验一致；
          // 机器人收发走 bot 身份，无需第二次用户授权）
          if (botCreate.bound.ok) {
            const sr = await listenerAction(ctx, "start");
            botCreate.connected = sr.ok && !/未运行|拒绝|失败|错误/.test(sr.output ?? "");
            ctx.logger.info(`lark-connector: 绑定成功，自动启动监听器 → ${sr.output || sr.error || "ok"}`);
          }
        } else {
...
async function botCreateStatus() {
  ...
  const base = {
    ...
    connected: botCreate.connected === true,
    ...
    try {
      ...
      if (method === "GET" && path === "/status") {
        ...
        json(res, 200, {
          ...
          poll: poller.status(),
          listener,
          workDir: process.env.DSH_WORK_DIR ?? "~",
        });
        return;
      }
      ...
      if (method === "POST" && (path === "/listener/start" || path === "/listener/stop")) {
        ...
        const r = await listenerAction(ctx, action);
        json(res, 200, r);
        return;
      }
