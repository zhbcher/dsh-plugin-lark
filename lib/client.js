...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var LarkConnectorPanel = function () {
      ...
      var create = react.useState({ phase: "idle", qrUrl: null, steps: [], logs: "", result: null, error: null, connected: false });
      ...
      var cancelCreate = react.useCallback(function () {
        api("/api/lark/bot/create/cancel", { method: "POST" }).then(function () {
          setCreateState({ phase: "idle", qrUrl: null, steps: [], logs: "", result: null, error: null, connected: false });
        });
      }, []);
          ...
          api("/api/lark/bot/create/status").then(function (d) {
            ...
            setCreateState(function (c) {
              ...
              if (d.phase === "done") {
                ...
                next.connected = d.connected === true;
      ...
      // 新机器人创建成功后：绑定+自动启动监听器已完成（服务端处理），
      // 无需自动跳转扫码授权；用户身份授权（扫码连接）为可选增强。
      ...
      return react.createElement("div", { style: s.section },
        ...
        // 授权卡（可选：用户身份授权）
        react.createElement("div", { style: s.card },
          react.createElement("h3", { style: s.h2 }, "用户身份授权（可选）"),
          react.createElement("p", { style: s.label },
            "连接机器人无需此步（机器人用应用身份收发消息）。仅当你需要以自己身份访问日历/文档/邮件/云盘等个人资源时，扫码授权。"),
          authState.phase === "idle" ?
            react.createElement("div", { style: s.row },
              react.createElement("button", { style: s.btn, onClick: startAuth }, "扫码授权"),
        ...
        react.createElement("div", { style: s.card },
          ...
          react.createElement("p", { style: s.label },
            "自动完成：创建应用 → 开启机器人 → 配置权限 → 订阅消息事件 → 创建测试版。扫一次码，约 30-60 秒新机器人自动连接。"),
            ...
            createState.error ?
              ...
              react.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } },
                createState.connected ?
                  react.createElement("p", { style: s.ok }, "✅ 已连接！机器人已创建（测试版自动生效），监听器已自动启动。直接去飞书发消息吧。") :
                  react.createElement("p", { style: s.ok }, "新机器人已创建 ✓ 已写入配置"),
                react.createElement("p", { style: s.label }, (createState.result?.appName || "") + " · AppID " + (createState.result?.appId || "")),
                createState.connected ? null :
                  react.createElement("p", { style: s.hint }, "监听器未自动启动，可点下方「启动监听」；或在「扫码连接」授权你的用户身份（可选，用于访问日历/文档/邮件等个人资源）"))),
